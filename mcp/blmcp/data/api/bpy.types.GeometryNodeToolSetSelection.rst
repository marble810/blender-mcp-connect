...
.. class:: GeometryNodeToolSetSelection(GeometryNode)
   ...
   .. attribute:: domain
      ...
      :type: Literal[:ref:`rna_enum_attribute_domain_point_edge_face_curve_items`]

   .. attribute:: selection_type

      (default ``'BOOLEAN'``)

      - ``BOOLEAN``
        Boolean -- Store true or false selection values in edit mode.
      - ``FLOAT``
        Float -- Store floating point selection values. For mesh geometry, stored inverted as the sculpt mode mask.

      :type: Literal['BOOLEAN', 'FLOAT']
